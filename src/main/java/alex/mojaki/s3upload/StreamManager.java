 ...
 * <p>
 * After creating an instance with details of the upload, use {@link StreamManager#getMultiPartOutputStreams()}
 * to get a list
 ...
 * <p>
 * Once all streams have been closed, call {@link StreamManager#complete()}. Alternatively you can call
 * {@link StreamManager#abort()}
 ...
 * <p>
 * You can configure the upload process by calling any of the chaining setter methods {@link StreamManager#numStreams(int)}, {@link StreamManager#numUploadThreads(int)}, {@link StreamManager#queueCapacity(int)}, or {@link StreamManager#partSize(long)} before calling {@code getMultiPartOutputStreams}. Parts that have been produced sit in a queue of specified capacity while they wait for a thread to upload them.
 ...
 * If you are uploading very large files, you may need to increase the part size - see {@link StreamManager#partSize(long)} for details.
 *
 * @author Alex Hall
 */
// @formatter:on
public class StreamManager {
    ...
    public StreamManager(String bucketName,
                         String putKey,
                         AmazonS3 s3Client) {
     ...
     * {@link StreamManager#getMultiPartOutputStreams()} for you to write to.
     * <p>
     ...
     * to avoid running out of part numbers - see {@link StreamManager#partSize(long)}
     * for more details.
     * <p>
     * Each stream may hold up to {@link StreamManager#partSize(long)} + 6MB
     * in memory at a time.
     *
     * @return this {@code StreamTransferManager} for chaining.
     * @throws IllegalArgumentException if the argument is less than 1.
     * @throws IllegalStateException    if {@link StreamManager#getMultiPartOutputStreams} has already
     *                                  been called, initiating the upload.
     */
    public StreamManager numStreams(int numStreams) {
        ensureCanSet();
        if (numStreams < 1) {
     ...
     * bandwidth to spare.
     * <p>
     * Each thread may hold up to {@link StreamManager#partSize(long)}
     * in memory at a time.
     *
     * @return this {@code StreamTransferManager} for chaining.
     * @throws IllegalArgumentException if the argument is less than 1.
     ...
     *                                  been called, initiating the upload.
     */
    public StreamManager numUploadThreads(int numUploadThreads) {
        ensureCanSet();
     ...
     * <p>
     * Each part sitting in the queue will hold {@link StreamManager#partSize(long)} bytes
     * in memory at a time.
     *
     * @return this {@code StreamTransferManager} for chaining.
     * @throws IllegalArgumentException if the argument is less than 1.
     ...
     *                                  been called, initiating the upload.
     */
    public StreamManager queueCapacity(int queueCapacity) {
        ensureCanSet();
        if (queueCapacity < 1) {
    ...
    public StreamManager partSize(long partSize) {
        ensureCanSet();
        partSize *= MB;
     ...
     *                               been called, initiating the upload.
     */
    public StreamManager checkIntegrity(boolean checkIntegrity) {
        ensureCanSet();
        if (checkIntegrity) {
    ...
    /**
     * Deprecated constructor kept for backward compatibility. Use {@link StreamManager#StreamManager(String, String, AmazonS3)} and then chain the desired setters.
     */
    @Deprecated
    public StreamManager(String bucketName,
                         String putKey,
     ...
     * by {@link StreamManager#getMultiPartOutputStreams()} to finish, then sends a request to S3 to complete
