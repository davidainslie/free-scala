  ...
  override def apply[A](fa: S3[A]): IO[A] =
    fa match {
      case CreateBucket(request, allowAlreadyExists) =>
        EitherT(IO.fromCompletableFuture(IO(s3Client.v2.async.createBucket(request))).attempt).valueOrF({
          case _: BucketAlreadyOwnedByYouException if allowAlreadyExists => IO(CreateBucketResponse.builder.build)
