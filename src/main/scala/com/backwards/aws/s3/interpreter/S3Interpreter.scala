...
object S3Interpreter {
  def apply(s3: S3Client): S3 ~> Id =
    new (S3 ~> Id) {
      override def apply[A](fa: S3[A]): Id[A] =
        fa match {
          case CreateBucket(request) =>
            s3.v2.sync.createBucket(request).asInstanceOf[A]

          case PutObject(request, body) =>
            s3.v2.sync.putObject(request, body).asInstanceOf[A]

          case PutStream(bucket, key) =>
            ??? // TODO

          case GetObject(request) =>
            s3.v2.sync.getObject(request).asInstanceOf[A]
        }
    }
}